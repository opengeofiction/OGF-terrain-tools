...
foreach my $querystr( @query )
{
	...
	my $fn = build_filename $SOURCE_DIR, $FN_ACTIVITY, $yesterday, "$DEGINCR°", $i++, 'csv';
	if( -e $fn )
	{
...
for( my $ii = 0; $ii <= $i; $ii++ )
{
	my $fn = build_filename $SOURCE_DIR, $FN_ACTIVITY, $yesterday, "$DEGINCR°", $ii, 'csv';
	if( open F, "<$fn" )
	{
		while( my $line = <F> )
		{
...
# create polygon JSON
my $polygon_fn = build_filename $SOURCE_DIR, $FN_POLYGONS, $yesterday, undef, undef, 'json';
my $first = 0;
...
my $activity_fn = build_filename $SOURCE_DIR, $FN_ACTIVITY, $yesterday, undef, undef, 'json';
$first = 0;
...
my $template_fn = build_filename $SOURCE_DIR, $FN_TEMPLATE, undef, undef, undef, 'json';
